...
GLuint loadSilhouShaders(const char *sil_vert_file_path, const char * sil_frag_file_path)
{
    ...
    std::string vertexShaderCode;
    std::ifstream vertexShaderStream(sil_vert_file_path, std::ios::in);
    if(vertexShaderStream.is_open())
    {
        std::string line = "";
        while(std::getline(vertexShaderStream, line))
    ...
    {
        Err(std::string("Couldn't open ") + sil_vert_file_path );
        exit(1);
    }

    std::string fragmentShaderCode;
    std::ifstream fragmentShaderStream(sil_frag_file_path, std::ios::in);
    if(fragmentShaderStream.is_open())
    {
        std::string line = "";
        while(std::getline(fragmentShaderStream, line))
    ...
    {
        Err(std::string("Couldn't open ") + sil_frag_file_path);
        exit(1);
    }
    ...
    Log(std::string("Compiling silhouette vertex shader ") + sil_vert_file_path);
    char const *vertexSource = vertexShaderCode.c_str();
    glShaderSource(vertexShaderID, 1, &vertexSource, NULL);
    glCompileShader(vertexShaderID);
    ...
    Log(std::string("Compiling silhouette fragment shader ") + sil_frag_file_path);
    char const *fragmentSource = fragmentShaderCode.c_str();
    glShaderSource(fragmentShaderID, 1, &fragmentSource, NULL);
    glCompileShader(fragmentShaderID);
    ...
    Log(std::string("Linking silhouette shader "));
    GLuint silhouProgramID = glCreateProgram();
    glAttachShader(silhouProgramID, vertexShaderID);
    glAttachShader(silhouProgramID, fragmentShaderID);
    glLinkProgram(silhouProgramID);

    glGetShaderiv(silhouProgramID, GL_LINK_STATUS, &result);
    glGetShaderiv(silhouProgramID, GL_INFO_LOG_LENGTH, &infoLogLength);
    if(infoLogLength > 0)
    {
        std::vector<char> programErrorMessage(infoLogLength + 1);
        glGetShaderInfoLog(silhouProgramID, infoLogLength, NULL, &programErrorMessage[0]);
        std::cout << programErrorMessage[0] << std::endl;
    }

    glDetachShader(silhouProgramID, vertexShaderID);
    glDetachShader(silhouProgramID, fragmentShaderID);

    glDeleteShader(vertexShaderID);
    glDeleteShader(fragmentShaderID);

    return silhouProgramID;
}
...
GLuint loadTessShaders(const char *tess_vert_file_path, const char *tess_ctrl_file_path, const char *tess_eval_file_path, const char *tess_frag_file_path)
{
	...
	Log("Compiling tessellation vertex shader: {}", tess_vert_file_path);
	...
	Log("Compiling tessellation control shader: {}\n", tess_ctrl_file_path);
	...
	Log("Compiling tessellation evaluation shader: {}", tess_eval_file_path);
	...
	printf("Compiling tessellation fragment shader: %s\n", tess_frag_file_path);
    ...
    printf("Linking tessellation Shader\n");
